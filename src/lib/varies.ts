...
const COLOR_OPTIONS = ['contrast', 'shade', 'tint'];
...
export function generateColorDynamic(handler: ColorOrGradientVaryHandler<any>) {
  return colorOrGradientVaries(
    ...
    (dynamic, option) => {
      ...
      const name = dynamic + suffix;
      const colorA = `var(--color-foreground${suffix})`;
      const colorB = `var(${
...
export function generateGradientDynamic(
  ...
  return colorOrGradientVaries(
    ...
    (dynamic, option) => {
      ...
      const name = dynamic + suffix;
      const colorA = `var(--color-foreground${suffix})`;
      const gradientA = `var(--gradient-foreground${suffix})`;
      const colorB = `var(${
        option === ColorOptions.Contrast
          ? '--color-foreground'
          : '--color-background'
